...
def recieve_request_set(info):
    ...
    ditry_column = info["ditry_column"]
    ...
    datalake_path = write_datalake_files(info["datalake"], index_name)
    ...
    # Index chosen but no name provided
    if index_type != "None" and index_name == None:
        raise ValueError("Index Name must be provided from retrieval")
...
def repair_table():
    ...
    ##### Ret + GPT - Scenario 2
        ...
        # Create the index
        main_create_index(params["datalake_path"], params["index_name"], chosen_index_type = params["index_type"])
