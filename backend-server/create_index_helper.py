...
def create_index(datalake_path, # Path to <FOLDER> containing all csv files
                 ...
                 tokenizer = None,
                 index_type = 'ES',
                 create_index_mode = "create_index"
                 ):
    ...
    # FAISS Branch
    if index_type == 'FAISS':
        dirpath = os.path.join(index_dir, index_name)
        # Overwrite existing if found
        if create_index_mode == "check_index" and os.path.exists(dirpath):
            return True
        elif create_index_mode == "create_index" and os.path.exists(dirpath):
        ...
        ds_with_embeddings.save_faiss_index('embeddings', dirpath)
...
#### MAIN FUNCTION
def main_create_index(datalake_path, index_name="es_index_1", chosen_index_type = "ES",index_dir = "./faiss_index/", create_index_mode = "no_retrieval"):
    ...
    if create_index_mode == "no_retrieval":
        return 
