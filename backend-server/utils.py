...
def write_datalake_files(list_of_csvs, index_name): # index_name is the folder name in ./datalake for this datalake
    if list_of_csvs == None or list_of_csvs == []:
        return None
    # Create temp folder name 
    if index_name == "" or index_name == None:
        index_name = "index_{}".format(time.strftime("%Y%m%d-%H%M%S"))
    
    # Index Already Exists
    if os.path.exists("./datalakes/{}".format(index_name)):
        return "./datalakes/{}".format(index_name)
